...
func (oc *SecondaryLayer2NetworkController) syncClusterRouterPorts(node *corev1.Node, hostSubnets []*net.IPNet) error {
	...
	if len(oc.remoteNodesNoRouter) > 0 {
		...
		return oc.setUDNLayer2NodeUsesTransitRouter(node.Name, oc.GetNetworkName(), true)
	}
}
...
func (oc *SecondaryLayer2NetworkController) cleanupUpgradeTopology() error {
...
func (oc *SecondaryLayer2NetworkController) setUDNLayer2NodeUsesTransitRouter(nodeName, netName string, remove bool) error {
	// Informer cache should not be mutated, so get a copy of the object
	resultErr := clientgoretry.RetryOnConflict(clientgoretry.DefaultBackoff, func() error {
		node, err := oc.watchFactory.NodeCoreInformer().Lister().Get(nodeName)
		if err != nil {
			return err
		}
		cnode := node.DeepCopy()
		updated, err := util.UpdateLayer2TransitRouterAnnotation(cnode.Annotations, oc.GetNetworkName(), remove)
		if err != nil {
			return fmt.Errorf("failed to update node %s with transit router annotation for network %s: %w", nodeName, netName, err)
		}
		if !updated {
			return nil
		}
		return oc.kube.UpdateNodeStatus(cnode)
	})
	return resultErr
}
