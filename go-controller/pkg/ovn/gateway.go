...
func GetNetworkScopedClusterSubnetSNATMatch(nbClient libovsdbclient.Client, netInfo util.NetInfo, nodeName string,
	...
	layer2NoRouter := Layer2NoRouterTopology(nbClient, netInfo)
...
func Layer2TransitRouterTopology(nbClient libovsdbclient.Client, netInfo util.NetInfo) bool {
	if netInfo.TopologyType() != types.Layer2Topology {
		return false
	}
	transitRouterName := netInfo.GetNetworkScopedClusterRouterName()
