...
if [ -f "triggered.data" ]; then
  RESPONSE_MESSAGE="It looks like the previous CI run failed, please report failed tests: this is the only way to get them fixed!
See https://ovn-kubernetes.io/ci/ci#ci-fails-what-do-i-do for more details on what to do.
The following workflows runs were successfully triggered:\n$(cat triggered.data)"
